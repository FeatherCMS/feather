...
fileprivate extension UniversalServer where APIHandler: APIProtocol {
    ...
    func formFieldRemove(
        ...
        try await handle(
            ...
            deserializer: { request, requestBody, metadata in
                let path: Operations.FormFieldRemove.Input.Path = .init(contactFormId: try converter.getPathParameterAsURI(
                    in: metadata.pathParameters,
                    name: "contactFormId",
                    as: Components.Parameters.ContactFormIdParameter.self
                ))
                let headers: Operations.FormFieldRemove.Input.Headers = .init(accept: try converter.extractAcceptHeaderIfPresent(in: request.headerFields))
                let contentType = converter.extractContentTypeIfPresent(in: request.headerFields)
                let body: Components.RequestBodies.DeleteRequestBody
                let chosenContentType = try converter.bestContentType(
                    received: contentType,
    ...
    func contactFormSubmissionRemove(
        ...
        try await handle(
            ...
            deserializer: { request, requestBody, metadata in
                let path: Operations.ContactFormSubmissionRemove.Input.Path = .init(contactFormId: try converter.getPathParameterAsURI(
                    in: metadata.pathParameters,
                    name: "contactFormId",
                    as: Components.Parameters.ContactFormIdParameter.self
                ))
