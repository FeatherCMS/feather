...
struct ContactFormSubmissionsTableContent: Component {
    ...
    func html(context: inout RenderContext) -> Div {
        let path = ContactAdminRoutes.formSubmissions(RouterPath(formId)).description
        ...
        return context.render(
            NewAdminList(
                table: {
                    if pageState.isPageOutOfRange { context.render(NewAdminListInvalidPageState(pageState: pageState, path: path)) }
                        ...
                        context.render(NewAdminListSelectionForm(
                            state: .init(action: ContactAdminRoutes.formSubmissionRemove(RouterPath(formId)).description, pageState: pageState, search: search, button: .init("Remove selected", style: .destructive), isEnabled: canDelete),
                            table: context.render(NewAdminListShell(
                                layout: .init(name: "contact-form-submissions", columns: [.fraction(1), .fraction(1), .fixed(150), .fixed(220)]), hasSelection: canDelete,
                                table: Table {
                                    Thead { Tr { if canDelete { context.render(NewAdminListSelectAllCheckbox()) }; Th("Submitted"); if hasEmailColumn { Th("Email") }; Th("Status"); Th("Actions") } }
                                    Tbody {
                                        for item in items {
                                            Tr {
                                                ...
                                                context.render(NewAdminListRowActions(label: "Actions", actions: [
                                                    .init("Details", href: ContactAdminRoutes.formSubmissionDetails(formID: RouterPath(formId), submissionID: RouterPath(item.id)).description, style: .ghost(.primary), permission: ContactPermissions.Submissions.read),
                                                    .init("Remove", href: NewAdminLocation.remove(path: ContactAdminRoutes.formSubmissionRemove(formID: RouterPath(formId), submissionID: RouterPath(item.id)).description, ids: [item.id], returnTo: returnTo), style: .destructive, permission: ContactPermissions.Submissions.delete),
                ...
                search: { context.render(NewAdminListSearch(state: .init(action: path, placeholder: "Quick search submissions", search: search))) },
