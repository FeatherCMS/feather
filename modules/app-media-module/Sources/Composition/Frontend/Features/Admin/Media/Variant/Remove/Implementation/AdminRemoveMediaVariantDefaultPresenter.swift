...
struct AdminRemoveMediaVariantDefaultPresenter: AdminRemoveMediaVariantPresenter
{
        ...
        async throws -> HTMLResponse
    {
        ...
        case .notFound:
            state = .init(
                title: "Media variants not found",
                message: "One or more selected variants may have been removed."
            )
        case .unauthorized:
            state = .init(
                title: "Session expired",
                message: "Please sign in again to remove media variants."
            )
        case .forbidden:
            state = .init(
                title: "Forbidden",
                message: "Your account cannot remove media variants."
            )
        case .unavailable:
            state = .init(
    ...
    func renderSuccess(location: String, count: Int) -> Response {
        AdminNotificationFlash.redirect(
            ...
            notification: .init(
                title: "Removed",
                message: count == 1
                    ? "Media variant removed successfully."
                    : "\(count) media variants removed successfully."
            )
        )
    }

    private func status(for error: AdminRemoveMediaVariantError)
        -> HTTPResponse.Status
    {
        switch error {
