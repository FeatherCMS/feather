...
struct AdminEditMediaVariantOpenAPIRepository: AdminEditMediaVariantRepository {
    ...
    func load(id: String) async throws -> MediaAdminAPI.Components.Schemas.MediaVariantDetailSchema {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.mediaVariantGet(path: .init(mediaVariantId: id), headers: .init(accept: [.init(contentType: .json)]))
            switch response {
    ...
    func addProcessor(variantId: String, input: MediaAdminAPI.Components.Schemas.MediaVariantProcessorCreateSchema) async throws {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.mediaVariantProcessorCreate(path: .init(mediaVariantId: variantId), headers: .init(accept: [.init(contentType: .json)]), body: .json(input))
            switch response {
            case .created: return
            case .notFound: throw OpenAPIRepositoryError.notFound
            case .unauthorized: throw OpenAPIRepositoryError.unauthorized
    ...
    func updateProcessor(variantId: String, id: String, input: MediaAdminAPI.Components.Schemas.MediaVariantProcessorCreateSchema) async throws {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.mediaVariantProcessorUpdate(path: .init(mediaVariantId: variantId, mediaVariantProcessorId: id), headers: .init(accept: [.init(contentType: .json)]), body: .json(input))
            switch response {
            case .ok: return
            case .notFound: throw OpenAPIRepositoryError.notFound
            case .unauthorized: throw OpenAPIRepositoryError.unauthorized
    ...
    func removeProcessor(variantId: String, id: String) async throws {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.mediaVariantProcessorRemove(path: .init(mediaVariantId: variantId), headers: .init(accept: [.init(contentType: .json)]), body: .json(.init(ids: [id], results: false, summary: true)))
            switch response {
            case .ok: return
            case .unauthorized: throw OpenAPIRepositoryError.unauthorized
            case .forbidden: throw OpenAPIRepositoryError.forbidden
