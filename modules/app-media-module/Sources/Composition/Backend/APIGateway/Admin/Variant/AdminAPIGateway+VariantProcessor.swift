...
extension AdminAPIGateway {
    public func mediaVariantProcessorGet(
        ...
        guard
            let result = try await useCases.makeGetVariantProcessor()
                .execute(
                    subject: subject,
                    input: .init(
    ...
    public func mediaVariantProcessorCreate(
        ...
        let subject = try await CurrentSubject.require()
        do {
            let result = try await useCases.makeCreateVariantProcessor()
                .execute(
                    subject: subject,
                    input: .init(
                        processor: .init(
    ...
    public func mediaVariantProcessorUpdate(
        ...
        let subject = try await CurrentSubject.require()
        do {
            let result = try await useCases.makeEditVariantProcessor()
                .execute(
                    subject: subject,
                    input: .init(
                        variantId: input.path.mediaVariantId,
    ...
    public func mediaVariantProcessorList(
        ...
        let query:
            Components.Schemas.MediaVariantProcessorListItemSearchQuerySchema
        switch input.body {
        case .json(let value): query = value
        }
        let subject = try await CurrentSubject.require()
    ...
    public func mediaVariantProcessorRemove(
        ...
        let subject = try await CurrentSubject.require()
        let deleted = try await useCases.makeRemoveVariantProcessor()
            .execute(
                subject: subject,
                input: .init(
                    variantId: input.path.mediaVariantId,
