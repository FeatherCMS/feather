...
extension AdminAPIGateway {
    ...
    func map(_ detail: MediaVariantDetail) -> MediaAdminAPI.Components.Schemas.MediaVariantDetailSchema {
        .init(id: detail.id, key: detail.key, name: detail.name, isRequired: detail.isRequired, isActive: detail.isActive, processors: detail.processors.map(map), createdAt: detail.createdAt.timeIntervalSince1970, updatedAt: detail.updatedAt.timeIntervalSince1970)
    }

    func map(_ item: MediaVariantList.Item) -> MediaAdminAPI.Components.Schemas.MediaVariantListItemSchema {
        .init(id: item.id, key: item.key, name: item.name, isRequired: item.isRequired, isActive: item.isActive)
    }

    func map(_ item: MediaVariantProcessorList.Item) -> MediaAdminAPI.Components.Schemas.MediaVariantProcessorListItemSchema {
        .init(id: item.id, name: item.name, matchExtensions: item.matchExtensions, commandTemplate: item.commandTemplate, isActive: item.isActive)
    }

    func map(_ processor: MediaVariantProcessor) -> MediaAdminAPI.Components.Schemas.MediaVariantProcessorDetailSchema {
        .init(id: processor.id, variantId: processor.variantId, name: processor.name, matchExtensions: processor.matchExtensions, commandTemplate: processor.commandTemplate, isActive: processor.isActive, createdAt: processor.createdAt.timeIntervalSince1970, updatedAt: processor.updatedAt.timeIntervalSince1970)
    }

    func map(_ query: MediaAdminAPI.Components.Schemas.MediaVariantListItemSearchQuerySchema) -> MediaVariantList.Query {
        .init(page: map(query.page), search: query.filters.search)
    }

    func map(_ query: MediaAdminAPI.Components.Schemas.MediaVariantProcessorListItemSearchQuerySchema) -> MediaVariantProcessorList.Query {
        .init(page: map(query.page), search: query.filters.search)
    }
}
