...
public struct TableSeedMigration: DatabaseMigration {
    ...
    public func apply(
        ...
        let processorRepository = MediaVariantProcessorDatabaseRepository(context: context)

        let variant: MediaVariant
        if var existing = try await variantRepository.list().first(where: { $0.key == "preview" }) {
        ...
        let existingProcessors = try await processorRepository.list(variantId: variant.id)
        for definition in definitions {
            if var existing = existingProcessors.first(where: { $0.name == definition.name }) {
