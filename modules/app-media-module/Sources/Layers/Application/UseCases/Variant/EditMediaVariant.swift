...
public struct EditMediaVariant: UseCase {
    struct Action: PermissionAction { let key = MediaPermissions.Variants.update }
    ...
    public struct Input: DTO {
        ...
        public init(id: String, variant: MediaVariantCreate) { self.id = id; self.variant = variant }
    }
    ...
    public func execute(subject: Subject, input: Input) async throws -> MediaVariantDetail {
        ...
        return try await transaction.run { scope in
            guard var variant = try await scope.variantDefinitions.find(id: input.id) else { throw Error.notFound }
            ...
            let processors = try await scope.variantProcessors.list(variantId: saved.id)
            return .init(id: saved.id, key: saved.key, name: saved.name, isRequired: saved.isRequired, isActive: saved.isActive, processors: processors.map(\.asVariantProcessorListItem), createdAt: saved.createdAt, updatedAt: saved.updatedAt)
        }
    }
}
