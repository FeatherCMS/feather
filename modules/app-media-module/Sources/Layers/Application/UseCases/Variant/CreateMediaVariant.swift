...
public struct CreateMediaVariant: UseCase {
    struct Action: PermissionAction { let key = MediaPermissions.Variants.create }
    ...
    public func execute(subject: Subject, input: Input) async throws -> MediaVariantDetail {
        ...
        let variant = try await transaction.run { scope in
            try await scope.variantDefinitions.insert(MediaVariant.create(key: input.variant.key, name: input.variant.name, isRequired: input.variant.isRequired, isActive: input.variant.isActive))
        }
        return .init(id: variant.id, key: variant.key, name: variant.name, isRequired: variant.isRequired, isActive: variant.isActive, processors: [], createdAt: variant.createdAt, updatedAt: variant.updatedAt)
    }
}
