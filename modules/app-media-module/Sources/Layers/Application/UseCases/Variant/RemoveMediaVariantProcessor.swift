...
public struct RemoveMediaVariantProcessor: UseCase {
    struct Action: PermissionAction { let key = MediaPermissions.VariantProcessors.delete }
    ...
    public struct Input: DTO { public let variantId: String; public let ids: [String]; public init(variantId: String, ids: [String]) { self.variantId = variantId; self.ids = ids } }
    public func execute(subject: Subject, input: Input) async throws -> [String] {
        ...
        return try await transaction.run { scope in
            let processors = try await scope.variantProcessors.list(variantId: input.variantId)
            let ids = Set(processors.map(\.id).filter { input.ids.contains($0) })
