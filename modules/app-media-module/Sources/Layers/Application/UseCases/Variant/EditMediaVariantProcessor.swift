...
public struct EditMediaVariantProcessor: UseCase {
    struct Action: PermissionAction { let key = MediaPermissions.VariantProcessors.update }
    ...
    public struct Input: DTO { public let variantId: String; public let id: String; public let processor: MediaVariantProcessorCreate; public init(variantId: String, id: String, processor: MediaVariantProcessorCreate) { self.variantId = variantId; self.id = id; self.processor = processor } }
    ...
    public func execute(subject: Subject, input: Input) async throws -> MediaVariantProcessor {
        ...
        return try await transaction.run { scope in
            guard var processor = try await scope.variantProcessors.find(id: input.id), processor.variantId == input.variantId else { throw Error.notFound }
