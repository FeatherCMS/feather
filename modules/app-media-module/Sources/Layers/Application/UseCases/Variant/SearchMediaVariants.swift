...
public struct SearchMediaVariants: UseCase {
    ...
    public struct Input: DTO { public let query: MediaVariantList.Query; public init(query: MediaVariantList.Query) { self.query = query } }

    public func execute(subject: Subject, input: Input) async throws -> MediaVariantList {
        let action = Action()
        ...
        let items = try await transaction.run { scope in
            var result = try await scope.variantDefinitions.list().map(\.asVariantListItem)
            if let search = input.query.search?.lowercased(), !search.isEmpty { result = result.filter { $0.key.lowercased().contains(search) || $0.name.lowercased().contains(search) } }
            return result
        }
    ...
    public func count(subject: Subject, input: Input) async throws -> Int {
        ...
        return try await transaction.run { scope in
            ...
            return items.filter { $0.key.lowercased().contains(search) || $0.name.lowercased().contains(search) }.count
        }
    }
}
