...
public struct SearchMediaVariantProcessors: UseCase {
    struct Action: PermissionAction {
        let key = MediaPermissions.VariantProcessors.list
    }
    ...
    public struct Input: DTO {
        ...
        public let query: MediaVariantProcessorList.Query
        public init(variantId: String, query: MediaVariantProcessorList.Query) {
        ...
        -> MediaVariantProcessorList
    {
        ...
        let items = try await transaction.run { scope in
            guard
                try await scope.variantDefinitions.find(id: input.variantId)
                    != nil
            else { throw Error.variantNotFound }
            var result = try await scope.variantProcessors
                .list(variantId: input.variantId)
                .map(\.asVariantProcessorListItem)
            if let search = input.query.search?.lowercased(), !search.isEmpty {
                result = result.filter {
                    $0.name.lowercased().contains(search)
                        || $0.matchExtensions.lowercased().contains(search)
                }
            }
            return result
        }
    ...
    public func count(subject: Subject, input: Input) async throws -> Int {
        ...
        return try await transaction.run { scope in
            ...
            let items = try await scope.variantProcessors.list(
