...
public struct RemoveMediaVariant: UseCase {
    struct Action: PermissionAction { let key = MediaPermissions.Variants.delete }
    ...
    public func execute(subject: Subject, input: Input) async throws -> [String] {
        ...
        return try await transaction.run { scope in try await scope.variantDefinitions.delete(ids: input.ids) }
    }
}
