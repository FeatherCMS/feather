...
public struct EditMediaFolder: UseCase {
    public enum Error: UseCaseError { case invalidName, notFound, duplicatePath }
    ...
    public func execute(subject: Subject, input: Input) async throws -> MediaFolderDetail {
        ...
        return try await transaction.run { scope in
            guard var folder = try await scope.folders.find(id: input.id) else { throw Error.notFound }
            let parentPath: String?
            if let parentId = folder.parentId { parentPath = try await scope.folders.find(id: parentId)?.slugPath }
            else { parentPath = nil }
            let slugPath = parentPath.map { "\($0)/\(slug)" } ?? slug
            if slugPath != folder.slugPath, try await scope.folders.find(slugPath: slugPath) != nil { throw Error.duplicatePath }
            folder.name = name; folder.slug = slug; folder.slugPath = slugPath
