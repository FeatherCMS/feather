...
public struct DeleteMediaFolder: UseCase {
    ...
    public func execute(
        ...
        let snapshot: FolderDeleteSnapshot? = try await transaction.run {
            ...
            let descendants = try await scope.folders.listDescendants(
                path: folder.path
            )
            let folderIds = descendants.map(\.id)
            let assets = try await scope.assets.list(folderIds: folderIds)
            var variantsByAssetId: [String: [MediaProcessorAsset]] = [:]
            for asset in assets {
                variantsByAssetId[asset.id] = try await scope.processorAssets
