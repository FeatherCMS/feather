...
public struct GenerateMediaAssetVariant: UseCase {
    ...
    public func execute(input: Input) async throws {
        let asset = try await transaction.run { scope in
            guard let asset = try await scope.assets.find(id: input.assetId) else {
                throw Error.assetNotFound
            }
            return asset
        }
        guard let processor = try await transaction.run({ scope in
            try await scope.variantProcessors.find(id: input.variantProcessorId)
        }) else {
        ...
        guard MediaExtensionMatcher.matches(extension: asset.extension, processor: processor) else {
            try await refreshStatus(assetId: asset.id)
            return
        }
        guard let variant = try await transaction.run({ scope in
        ...
        if try await transaction.run({ scope in
            try await scope.variants.find(nodeId: asset.id, variantId: variant.id)
        }) != nil {
            try await refreshStatus(assetId: asset.id)
            return
        }
...
private extension GenerateMediaAssetVariant {
    ...
    func runProcessor(
        ...
        guard result.exitCode == 0 else {
            throw Error.commandFailed(processorName: processor.name, command: command, status: result.exitCode, stderr: result.standardError)
        }
        ...
        return .init(
            ...
            extension: MediaExtensionMatcher.canonicalExtension(from: resolvedURL.pathExtension) ?? inputExtension
        )
    }
    ...
    func refreshStatus(assetId: String) async throws {
        ...
        let requiredVariants = try await transaction.run { scope in
            try await scope.variantDefinitions.listActive().filter { $0.isRequired }
        }
        let applicableProcessors = try await transaction.run { scope in
            try await scope.variantProcessors.listActive().filter {
                MediaExtensionMatcher.matches(extension: asset.extension, processor: $0)
            }
        }
        let generated = Set(try await transaction.run { scope in
