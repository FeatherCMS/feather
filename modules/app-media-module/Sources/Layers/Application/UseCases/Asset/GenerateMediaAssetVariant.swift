...
public struct GenerateMediaAssetVariant: UseCase {
    ...
    public func execute(
        ...
        let asset = try await transaction.run { scope in
            guard let found = try await scope.assets.find(id: input.assetId)
            else {
        ...
        let processor = try await transaction.run { scope in
            try await scope.processors.find(id: input.processorId)
        }
        ...
        let alreadyLinked = try await transaction.run { scope in
            try await scope.processorAssets.find(
                assetId: asset.id,
        ...
        _ = try await transaction.run { scope in
            try await scope.processorAssets.insert(
...
extension GenerateMediaAssetVariant {
    ...
    fileprivate func refreshAssetStatus(
        ...
        let asset = try await transaction.run { scope in
            guard let found = try await scope.assets.find(id: assetId) else {
                throw Error.assetNotFound
            }
            return found
        }

        let processors = try await transaction.run { scope in
            try await scope.processors.listActive()
                .filter {
                    MediaExtensionMatcher.matches(asset: asset, processor: $0)
                }
        }

        let linkedProcessorIds = try await transaction.run { scope in
            Set(
                try await scope.processorAssets.list(assetId: asset.id)
                    .map(\.processorId)
            )
        }
        ...
        _ = try await transaction.run { scope in
            try await scope.assets.update(updatedAsset)
        }
    }
