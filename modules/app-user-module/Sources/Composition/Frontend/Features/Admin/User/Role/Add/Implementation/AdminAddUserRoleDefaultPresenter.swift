...
struct AdminAddUserRoleDefaultPresenter: AdminAddUserRolePresenter {
    ...
    private func renderAddPage(state: UserRoleForm.State, status: HTTPResponse.Status) async throws -> HTMLResponse {
        let nonceToken = await AdminNonceStore.shared.issue(sessionToken: context.sessionToken)
        return try await renderPage(content: UserRoleAddPage(form: state, nonceToken: nonceToken), status: status)
    }
    ...
    func renderAddError(input: AdminAddUserRoleFormInput?, error: AdminAddUserRoleError) async throws -> HTMLResponse {
        ...
        case .conflict: return try await renderFormError(input: input, message: "A user role with this ID already exists.", status: .conflict)
        case .unavailable: return try await renderFormError(input: input, message: "The user role could not be created. Please try again.", status: .serviceUnavailable)
        }
    }

    func renderSuccess() -> Response {
        AdminNotificationFlash.redirect(to: UserRoleRoutes.list.description, notification: .init(title: "Added", message: "User role added successfully."))
    }

    func renderUnauthorizedPage() async throws -> HTMLResponse {
        try await renderStatusPage(title: "Session expired", message: "Please sign in again to create user roles.", status: .unauthorized)
    }

    func renderForbiddenPage() async throws -> HTMLResponse {
        try await renderStatusPage(title: "Forbidden", message: "Your account cannot create user roles.", status: .forbidden)
    }
    ...
    private func renderFormError(input: AdminAddUserRoleFormInput?, message: String, status: HTTPResponse.Status) async throws -> HTMLResponse {
        var state = input.map(UserRoleForm.State.from(input:)) ?? .addEmpty()
        state.error = message
