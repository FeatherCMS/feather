...
struct AdminListUserIdentityDefaultPresenter: AdminListUserIdentityPresenter {
    ...
    func renderListPage(model: NewAdminListModel<Components.Schemas.UserIdentityListItemSchema>, permissions: Set<PermissionKey>, search: String?, role: String?) async throws -> HTMLResponse {
        try await renderingEngine.renderNewAdminPage(request: request, context: context, title: "User identities", content: UserIdentityTable(permissions: NewAdminListActions(permissions), identities: model.items, pageState: model.pageState, search: search, role: role))
    }
    ...
    func renderErrorPage(error: AdminListUserIdentityError) async throws -> HTMLResponse {
        ...
        case .unavailable: state = .init(title: "User identities unavailable", message: "The request could not be completed. Please try again.")
        }
        let page = try await renderingEngine.renderNewAdminPage(request: request, context: context, title: "User identities", content: NewAdminStatusView(state: state, icon: FeatherIcons.alertCircle()))
        return HTMLResponse(content: page.content, status: status(for: error))
    }

    private func status(for error: AdminListUserIdentityError) -> HTTPResponse.Status {
        switch error {
