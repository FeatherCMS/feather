...
struct AdminViewUserOverviewComponent: Component {
    struct Destination {
        let title: String
    ...
    private let destinations = [
        Destination(
            ...
            description: "Manage identities and their assigned roles.",
            href: UserIdentityRoutes.list.description,
            icon: "users"
        ),
        Destination(
            title: "User roles",
            description: "Manage roles assigned to user identities.",
            href: UserRoleRoutes.list.description,
            icon: "shield"
        ),
    ]
    ...
    func rules() -> [any Rule] {
        Media {
            Custom(".user-overview-destinations") {
                Margin(top: 8.px)
                RowGap(16.px)
                ColumnGap(16.px)
            }
            ...
            Custom(
                ".user-overview-destination h2, .user-overview-destination p"
            ) { Margin(0) }
            Custom(".user-overview-destination h2 + p") { Margin(top: (-6).px) }
            Custom(".user-overview-destination p") {
                Color(.variable(TokenKey.Colors.Materials.Tertiary.text))
                LineHeight(1.5)
            }
            Custom(".user-overview-destination .new-admin-button") {
                Margin(top: 4.px)
            }
        }
    }
