...
struct NewsletterIssueAddView: Component {
    ...
    func html(context: inout RenderContext) -> some BasicTag {
        Section {
            ...
            .action(
                state.issueId.map {
                    "/admin/newsletter/\(state.newsletterId)/issues/\($0)/edit/"
                } ?? "/admin/newsletter/\(state.newsletterId)/issues/add/"
            )
            .class("cms-form")
            ...
            Div {
                Div {
                    ...
                    .action(
                        state.issueId.map {
                            "/admin/newsletter/\(state.newsletterId)/issues/\($0)/test-email/"
                        }
                            ?? "/admin/newsletter/\(state.newsletterId)/issues/test-email/"
                    )
                    .class("cms-form")
                }
