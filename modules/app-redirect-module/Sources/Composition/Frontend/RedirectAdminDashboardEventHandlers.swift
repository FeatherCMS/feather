    ...
    public static func register(in events: inout EventRegistry) {
        events.register(
            ...
            guard context.permissions.contains("redirect:rules:list") else {
                return [.init(contentStats: [], dailyTraffic: nil, topPages: nil, insightCards: [])]
            }

            let count = try? await api.withOpenAPIRepositoryErrorMapping { client in
                let response = try await client.redirectRuleSearch(
                    headers: .init(accept: [.init(contentType: .json)]),
                    body: .json(.init(page: .init(size: 1, number: 1), filters: .init(search: nil, statusCode: nil)))
                )
                switch response {
            ...
            return [.init(
                contentStats: count.map { [.init(label: "Redirect rules", value: "\($0)")] } ?? [],
                dailyTraffic: nil,
                topPages: nil,
                insightCards: []
            )]
