...
struct RedirectRuleForm: Component {
    struct State: Sendable {
        ...
        static func empty() -> Self {
            .init(
                source: .init(name: "source", label: "Source path", isRequired: true),
                destination: .init(name: "destination", label: "Destination URL or path", isRequired: true),
                statusCode: .init(
                    name: "statusCode",
        ...
        static func from(input: RedirectRuleFormInput) -> Self {
        ...
        static func from(rule: RedirectRuleDetailsModel) -> Self {
            var state = Self.empty()
            state.source.value = rule.source
            state.destination.value = rule.destination
            state.statusCode.value = String(rule.statusCode.rawValue)
            state.notes.value = rule.notes ?? ""
            return state
        }

        private static let statusOptions = [
            NewAdminFormFieldSelect.SelectOption(label: "301 Moved Permanently", value: "301"),
            NewAdminFormFieldSelect.SelectOption(label: "302 Found", value: "302"),
            NewAdminFormFieldSelect.SelectOption(label: "307 Temporary Redirect", value: "307"),
            NewAdminFormFieldSelect.SelectOption(label: "308 Permanent Redirect", value: "308"),
        ]
    }
