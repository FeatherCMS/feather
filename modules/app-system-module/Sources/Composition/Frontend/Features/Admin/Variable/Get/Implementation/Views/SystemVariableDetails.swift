...
struct SystemVariableDetails: Component {
    ...
    func html(context: inout RenderContext) -> some BasicTag {
        context.render(NewAdminDetailView(
                ...
                actions: [
                    .init(label: "Edit", href: SystemVariableRoutes.edit(RouterPath(state.variable.id)).description, style: .primary),
                    .init(label: "Remove", href: SystemVariableRoutes.removeFromDetails(state.variable.id), style: .destructive)
                ]
            ))
    }
}
