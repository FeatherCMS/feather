...
struct AdminGetDashboardComponent: Component {
    private let chartPrimaryColor = "var(--cms-primary-hover)"

    let model: AdminGetDashboardModel
    ...
    private func dailyTrafficCard(
        _ points: [AdminGetDashboardModel.TrafficPoint]
    ) -> some FlowContent {
        Div {
            H3("Daily traffic")
                .style("margin:0 0 16px 0;")
    ...
    private func breakdownCard(
        _ title: String,
        _ items: [AdminGetDashboardModel.BreakdownItem]
    ) -> some FlowContent {
        Div {
            H3(title)
                .style("margin:0 0 16px 0;")
