...
struct AccountInfrastructureTestSuite {
    ...
    func accountProfileMigrationAndRepositoryRoundTrip() async throws {
        ...
        let client = PostgresClient(
            configuration: .init(
                host: ProcessInfo.processInfo.environment["POSTGRES_HOST"] ?? "127.0.0.1",
                port: Int(ProcessInfo.processInfo.environment["POSTGRES_PORT"] ?? "55435") ?? 55435,
                ...
                database: ProcessInfo.processInfo.environment["POSTGRES_DB"] ?? "postgres",
        ...
        try await database.withTransaction { connection in
            ...
            try await AccountInfrastructure.TableMigration(connection: connection)
                .apply(on: connection)
            ...
            let repository = AccountProfileDatabaseRepository(
                context: .init(connection: connection, idGenerator: TestIDGenerator())
            )
            ...
            let invitation = try await InvitationTable(connection: connection).save(
                row: .init(
                    id: "invitation-1",
                    userId: "account-1",
            ...
            let persistedInvitation = try await InvitationTable(connection: connection)
                .find(token: invitation.token)
            #expect(persistedInvitation?.roleIDs == ["role-editor", "role-author"])
