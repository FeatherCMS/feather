...
extension WebMetadataTable.Row {
    ...
    init(from row: DatabaseRow) throws {
        ...
        self.template = try row.decode(column: "template", as: String.self)
        ...
        try await connection.run(
            query: """
                ...
                VALUES (
                    ...
                    \(row.template),
                    \(row.slug),
                    TO_TIMESTAMP(\(row.publicationDate.timeIntervalSince1970)),
    ...
    func list(
        ...
        try await connection.run(
            query: #"""
                ...
                WHERE (
                    (\#(referenceType == nil)
                        OR reference_type = \#(referenceType ?? ""))
                    AND (\#(search == nil)
        ...
        try await connection.run(
            query: #"""
                ...
                SET
                    ...
                    publication_date=TO_TIMESTAMP(\#(row.publicationDate.timeIntervalSince1970)),
