...
public enum WebAdminDashboardEventHandlers {
    public static func register(in events: inout EventRegistry) {
        events.register(
            ...
            await appendCount(label: "Web pages", permission: "web:pages:list", permissions: context.permissions, operation: { try await countPages(using: api) }, to: &contentStats)
            await appendCount(label: "Web menus", permission: "web:menus:list", permissions: context.permissions, operation: { try await countMenus(using: api) }, to: &contentStats)

            return [.init(contentStats: contentStats, dailyTraffic: nil, topPages: nil, insightCards: [])]
        }
    }

    private static func countPages(using api: WebAdminAPIClient) async throws -> Int {
    ...
    private static func countMenus(using api: WebAdminAPIClient) async throws -> Int {
        try await api.withOpenAPIRepositoryErrorMapping { client in
