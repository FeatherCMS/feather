...
extension BlogModule {
    ...
    func installSampleContent(_ req: Request) -> EventLoopFuture<Void> {
        ...
        return req.db.eventLoop.flatten([
            [c1].create(on: req.db),
            try! c1.updateContent(on: req.db) { content in
                content.status = .published
                content.date = now
            },
            [a1].create(on: req.db),
            try! a1.updateContent(on: req.db) { content in
                content.status = .published
                content.date = now
            },
            [p1, p2, p3, p4, p5].create(on: req.db),
            try! p1.updateContent(on: req.db) { content in
                content.status = .published
                content.date = now
            },
            try! p2.updateContent(on: req.db) { content in
                content.status = .published
                content.date = Date(timeInterval: -1*86400, since: now)
            },
            try! p3.updateContent(on: req.db) { content in
                content.status = .published
                content.date = Date(timeInterval: -2*86400, since: now)
            },
            try! p4.updateContent(on: req.db) { content in
                content.status = .published
                content.date = Date(timeInterval: -3*86400, since: now)
            },
            try! p5.updateContent(on: req.db) { content in
                content.status = .published
