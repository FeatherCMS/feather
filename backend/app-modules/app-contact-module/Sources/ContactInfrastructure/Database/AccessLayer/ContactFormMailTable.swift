...
extension ContactFormMailTable.Row {
    init(from row: DatabaseRow) throws {
        ...
        additionalHeaders = try row.decode(column: "additional_headers", as: String.self)
...
struct ContactFormMailTable {
    ...
    func create(model: ContactFormMail.New) async throws -> Row {
        try await connection.run(query: #"""
            INSERT INTO contact_form_mail (
                id, form_id, mail_from, mail_to, subject,
                additional_headers, message_body, created_at, updated_at
            ) VALUES (
                \#(model.id), \#(model.formId), \#(model.mailFrom), \#(model.mailTo),
                \#(model.subject), \#(model.additionalHeaders), \#(model.messageBody), NOW(), NOW()
            ) RETURNING *;
            """#) { sequence in
            guard let row = try await sequence.collect().first else { throw RepositoryError.notFound }
            return try Row(from: row)
        }
    }

    func delete(formId: String) async throws {
        try await connection.run(query: #"DELETE FROM contact_form_mail WHERE form_id = \#(formId);"#) { _ in }
    }
}
