...
public struct CreateContactForm: UseCase {
    ...
    public func execute(_ input: Input) async throws -> ContactFormDetail {
        try await transaction.run { context in
            let model = try ContactForm.create(id: idGenerator.generate(), name: input.name, successMessage: input.successMessage, failureMessage: input.failureMessage, redirectUrl: input.redirectUrl)
            ...
            let items = try await context.item.listBy(formId: saved.id).map(\.asDetail)
            for mail in input.mails {
                _ = try await context.mail.insert(.init(
                    id: idGenerator.generate(), formId: saved.id,
                    mailFrom: mail.mailFrom, mailTo: mail.mailTo,
                    subject: mail.subject, additionalHeaders: mail.additionalHeaders,
                    messageBody: mail.messageBody
                ))
            }
            let mails = try await context.mail.listBy(formId: saved.id).map(\.asDetail)
