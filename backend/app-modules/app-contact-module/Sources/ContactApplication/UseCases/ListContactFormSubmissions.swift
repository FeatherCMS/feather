...
public struct ListContactFormSubmissions: UseCase {
    ...
    public struct Input: DTO { public let formId: String; public init(formId: String) { self.formId = formId } }
    public func execute(_ input: Input) async throws -> [ContactFormSubmissionDetail] {
        try await transaction.run { context in try await context.submission.listBy(formId: input.formId).map(\.asDetail) }
    }
}
