...
public struct CreateNewsletterSubscriber: UseCase {
    ...
    public struct Input: DTO {
        ...
        public init(newsletterId: String, email: String, firstName: String = "", lastName: String = "", status: NewsletterCampaignSubscriber.Status = .subscribed) { self.newsletterId = newsletterId; self.email = email; self.firstName = firstName; self.lastName = lastName; self.status = status }
    }
    ...
    public func execute(_ input: Input) async throws -> NewsletterSubscriberDetail {
        ...
        return try await transaction.run { context in
            let value = try NewsletterCampaignSubscriber.create(newsletterId: input.newsletterId, email: input.email, subscriptionDate: now, firstName: input.firstName, lastName: input.lastName)
