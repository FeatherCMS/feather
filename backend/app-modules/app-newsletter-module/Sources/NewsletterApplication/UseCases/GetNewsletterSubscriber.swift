...
public struct GetNewsletterSubscriber: UseCase {
    ...
    public struct Input: DTO {
        ...
        public init(newsletterId: String, email: String) { self.newsletterId = newsletterId; self.email = email }
    }

    public func execute(_ input: Input) async throws -> NewsletterSubscriberDetail {
        try await transaction.run { context in
            guard let value = try await context.subscriber.findBy(newsletterId: input.newsletterId, email: input.email) else { throw Error(message: "Newsletter subscriber not found") }
