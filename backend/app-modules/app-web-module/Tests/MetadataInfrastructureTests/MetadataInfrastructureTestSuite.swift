...
@Suite
struct MetadataInfrastructureTestSuite {
    ...
    func example() async throws {
        ...
        let transaction = DatabaseTransactionExecutor(database: database) {
            WriteMetadata(
                metadata: DatabaseMetadataRepository(connection: $0)
            )
        }
        ...
        let query = DatabaseQueryExecutor(database: database) {
            GenericScope(
                [
                    "entry": DatabaseMetadataRepository(connection: $0)
                ]
            )
            //            EntryContext(
            //                metadata: DatabaseMetadataRepository(connection: $0)
        ...
        let entry = try await query.run { ctx in
            ctx.entry
        }
        print(entry)

        let useCase2 = AddMetadata(
            authorizer: authorizer,
            transaction: transaction,
            idGenerator: idGenerator
        )
