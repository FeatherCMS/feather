...
extension AdminAPI {
    func contactNewsletterSubscriberUpdate(
        _ input: Operations.ContactNewsletterSubscriberUpdate.Input
    ) async throws -> Operations.ContactNewsletterSubscriberUpdate.Output {
        try await modules.newsletter.authorize(
            permission: NewsletterPermissions.Subscribers.update
        )
        ...
        let result = try await modules.newsletter
            ...
            .execute(
                .init(
                    ...
                    status: NewsletterCampaignSubscriber.Status(
