...
extension AdminAPI {
    func contactNewsletterUpdate(_ input: Operations.ContactNewsletterUpdate.Input) async throws -> Operations.ContactNewsletterUpdate.Output {
        ...
        let current = try await modules.newsletter.makeGetNewsletter().execute(.init(id: input.path.contactNewsletterId))
        let result = try await modules.newsletter.makeUpdateNewsletter().execute(.init(id: current.id, name: body.name ?? current.name, fromEmail: body.fromEmail ?? current.fromEmail))
