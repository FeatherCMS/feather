...
extension AdminAPI {
    func contactNewsletterIssueList(
        ...
        try await modules.newsletter.authorize(permission: NewsletterPermissions.Issues.list)
        let result = try await modules.newsletter.makeListNewsletterIssues().execute(
