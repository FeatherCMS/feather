...
extension AdminAPI {
    func contactFormSubmissionList(
        ...
        try await modules.contact.authorize(
            permission: ContactPermissions.Submissions.list
        )
        let result = try await modules.contact.makeListContactFormSubmissions()
            .execute(.init(formId: input.path.contactFormId))
