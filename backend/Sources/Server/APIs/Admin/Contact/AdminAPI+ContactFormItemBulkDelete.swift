...
extension AdminAPI {
    func contactFormItemBulkDelete(
        _ input: Operations.ContactFormItemBulkDelete.Input
        ...
        let results: [Components.Schemas.BulkDeleteResponseSchema.ResultsPayloadPayload] = try await body.ids.asyncMap { id in
            do {
                try await useCase.execute(.init(id: id, formId: input.path.contactFormId))
                deleted += 1
        ...
        return .ok(.init(body: .json(.init(
            results: results,
            summary: .init(requested: body.ids.count, deleted: deleted, notFound: notFound, forbidden: 0)
        ))))
    }
}
