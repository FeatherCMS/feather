    ...
    if (document.querySelector('#markdownInput')) {
        ...
        function parseMarkdown(markdown) {
          return splitMarkdownBlocks(markdown).filter(Boolean).map(part => {
            ...
            const newsletter = part.match(/^@NewsletterCampaign\(id:\s*([^\)]+)\)$/); if (newsletter) return newBlock('newsletter', newsletter[1].trim());
            const contactForm = part.match(/^@ContactForm\(id:\s*([^\)]+)\)$/); if (contactForm) return newBlock('contact-form', contactForm[1].trim());
        ...
        function renderEmbedControl(body, block, update) {
          ...
          const id = document.createElement('input'); id.value = block.value || ''; id.placeholder = block.type === 'newsletter' ? 'Newsletter campaign ID' : 'Contact form ID'; id.setAttribute('aria-label', id.placeholder); id.addEventListener('input', event => update('value', event.target.value));
          ...
          fields.append(id, picker); body.append(fields);
        }
