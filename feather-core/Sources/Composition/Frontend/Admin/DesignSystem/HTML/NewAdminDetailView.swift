...
public struct NewAdminDetailView: Component {
    ...
    public struct Action: Sendable {
        ...
        public init(label: String, href: String, style: NewAdminButtonStyle) {
            self.label = label; self.href = href; self.style = style
        }
    }
    ...
    public init(breadcrumb: NewAdminBreadcrumb.State, pageHeader: NewAdminPageHeader.State, fields: [Field], actions: [Action] = []) {
        self.breadcrumb = breadcrumb; self.pageHeader = pageHeader; self.fields = fields; self.actions = actions
    }
            ...
            if !actions.isEmpty {
                Div {
                    for action in actions {
                        context.render(NewAdminButton(action.label, href: action.href, style: action.style))
                    }
    ...
    public func rules() -> [any Rule] {
        Media {
            Custom(".admin-detail-view-fields") { Display(.grid); Gap(12.px) }
            ...
            Custom(".admin-detail-view-field-value") {
                Margin(top: 6.px); Padding(vertical: 10.px, horizontal: 12.px)
