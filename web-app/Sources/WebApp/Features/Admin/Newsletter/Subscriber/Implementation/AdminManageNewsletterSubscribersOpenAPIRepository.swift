...
struct AdminManageNewsletterSubscribersOpenAPIRepository {
    let api: AdminAPI
    ...
    func create(newsletterId: String, form: NewsletterSubscriberForm) async throws {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.contactNewsletterSubscriberCreate(path: .init(contactNewsletterId: newsletterId), body: .json(.init(email: form.email, firstName: form.firstName, lastName: form.lastName, status: form.status)))
            switch response {
            case .created: return
            case .unauthorized: throw OpenAPIRepositoryError.unauthorized(message: "Please sign in again to create subscribers.")
            case .forbidden: throw OpenAPIRepositoryError.forbidden(message: "Your account cannot create subscribers.")
            case .undocumented(let statusCode, let response): throw try await api.failure(statusCode: statusCode, responseBody: response.body)
            }
        }
    }
    ...
    func update(newsletterId: String, email: String, form: NewsletterSubscriberForm) async throws {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            ...
            case .ok: return
            case .notFound: throw OpenAPIRepositoryError.notFound(message: "This subscriber could not be found.")
            case .unauthorized: throw OpenAPIRepositoryError.unauthorized(message: "Please sign in again to edit subscribers.")
            case .forbidden: throw OpenAPIRepositoryError.forbidden(message: "Your account cannot edit subscribers.")
            case .undocumented(let statusCode, let response): throw try await api.failure(statusCode: statusCode, responseBody: response.body)
            }
        }
    }
