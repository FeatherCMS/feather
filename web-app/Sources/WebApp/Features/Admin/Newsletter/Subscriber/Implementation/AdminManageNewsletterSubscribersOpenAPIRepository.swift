...
struct AdminManageNewsletterSubscribersOpenAPIRepository {
    ...
    {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.contactNewsletterSubscriberCreate(
                path: .init(contactNewsletterId: newsletterId),
                body: .json(
                    .init(
                        email: form.email,
        ...
        -> AdminManageNewsletterSubscriberItem
    {
        ...
        return try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.contactNewsletterSubscriberGet(
                path: .init(contactNewsletterId: newsletterId, email: email)
            )
            switch response {
            case .ok(let value):
                let item = try value.body.json
                return AdminManageNewsletterSubscriberItem(
                    id: item.id,
                    email: item.email,
    ...
    func update(
        ...
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.contactNewsletterSubscriberUpdate(
                path: .init(contactNewsletterId: newsletterId, email: email),
                body: .json(
                    .init(
                        firstName: form.firstName,
    ...
    func remove(newsletterId: String, subscriberId: String) async throws {
        ...
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.contactNewsletterSubscriberDelete(
                path: .init(contactNewsletterId: newsletterId, email: email)
            )
            switch response {
