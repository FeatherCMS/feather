            ...
            if state.items.isEmpty { P(state.search.isEmpty ? "No subscribers yet." : "No subscribers match your search.") } else {
                ListTableBulkRemoveForm(
                    ...
                    table: ListTableShell(table: Table {
                        ...
                        Tbody { for item in state.items { Tr { if state.canRemove { ListTableRowSelectCheckbox(state: .init(id: item.id)) }; Td(item.email).data("label", "Email"); Td("\(item.firstName) \(item.lastName)").data("label", "Name"); Td(item.status).data("label", "Status"); ListTableRowActions(state: .init(label: "Actions", actions: [.init(title: "Edit", href: "/admin/newsletters/\(state.newsletterId)/subscribers/\(item.id)/edit/", className: "edit", permission: "newsletter:subscribers:update"), .init(title: "Remove", href: "/admin/newsletters/\(state.newsletterId)/subscribers/\(item.id)/remove/", className: "delete", permission: "newsletter:subscribers:delete")], permissions: ["newsletter:subscribers:update", "newsletter:subscribers:delete"])) } } }
