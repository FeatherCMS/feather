...
struct AdminNewsletterSubscribersDirectoryDefaultPresenter {
    ...
    func render(
        model: AdminNewsletterSubscribersDirectoryModel,
        error: String?,
    ...
    func renderBulkRemoveConfirmation(
        ...
        let query =
            campaignId?.isEmpty == false ? "?campaignId=\(campaignId!)" : ""
        return renderEngine.renderAdminPage(
            ...
            content: ListBulkRemoveConfirmation(
                state: .init(
                    ...
                    action:
                        "/admin/newsletters/subscribers/bulk-remove/\(query)",
                    cancelLink: "/admin/newsletters/subscribers/\(query)",
                    selectedIds: emails
                )
            )
        )
