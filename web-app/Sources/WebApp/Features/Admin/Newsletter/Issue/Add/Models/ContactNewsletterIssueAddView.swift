...
struct ContactNewsletterIssueAddView: Component {
    struct State { let subject: String; let content: String; let scheduledAt: String; let newsletterId: String; let issueId: String?; let error: String?; let breadcrumb: AdminBreadcrumb.State }
    let state: State
    func content() -> some BasicTag {
        Section {
            AdminNewsletterCampaignTabs(campaignId: state.newsletterId, active: .issues)
            AdminBreadcrumb(state: state.breadcrumb)
            H1(state.issueId == nil ? "Add campaign issue" : "Edit campaign issue")
            ...
            Form {
                ...
                Div { Button(state.issueId == nil ? "Add" : "Save").type(.submit) }.class("button-row")
            }.method(.post).action(state.issueId.map { "/admin/newsletters/\(state.newsletterId)/issues/\($0)/edit/" } ?? "/admin/newsletters/\(state.newsletterId)/issues/add/").class("cms-form")
            ...
            Div {
                Div {
                    Div {
                        H3("Send test email")
                        Button("Close").type(.button).class("ghost").data("newsletter-test-email-close", "newsletterTestEmailModal")
                ...
                Form {
                    ...
                    Label { AdminFieldLabel(label: "Test email address", required: true); Input().type(.email).class("text-input").name("email").required() }
                    Div { Button("Send").type(.submit) }.class("button-row")
                }.method(.post).action(state.issueId.map { "/admin/newsletters/\(state.newsletterId)/issues/\($0)/test-email/" } ?? "/admin/newsletters/\(state.newsletterId)/issues/test-email/").class("cms-form")
                }.class("newsletter-test-email-lightbox-card")
            }.id("newsletterTestEmailModal").class("newsletter-test-email-lightbox")
            Style("""
