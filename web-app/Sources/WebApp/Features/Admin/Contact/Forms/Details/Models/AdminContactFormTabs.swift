...
struct AdminContactFormTabs: Component, FlowContent {
    ...
    func content() -> some BasicTag {
        Div {
            ...
            A("Details").href("/admin/contact/forms/\(formId)/details/")
                .if(active == .details) { $0.class("is-current") }
            A("Emails").href("/admin/contact/forms/\(formId)/emails/")
                .if(active == .emails) { $0.class("is-current") }
            A("Submissions").href("/admin/contact/forms/\(formId)/submissions/")
                .if(active == .submissions) { $0.class("is-current") }
        }
