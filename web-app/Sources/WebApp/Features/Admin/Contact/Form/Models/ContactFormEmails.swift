...
struct ContactFormEmails: Component {
    ...
    func content() -> some BasicTag {
        Section {
            ...
            Div { AdminNavigationButton("Add email", href: "/admin/contact/forms/\(id)/emails/add/") }.class("button-row")
            Br(); Br()
            if mails.isEmpty {
                P("No emails configured yet.")
            } else {
                ListTableShell(table: Table {
                    Thead { Tr { Th("From"); Th("To"); Th("Subject"); Th("Actions") } }
                    Tbody {
                        for mail in mails {
                            Tr {
                                ...
                                ListTableRowActions(state: .init(label: "Actions", actions: [
                                    .init(title: "Edit", href: "/admin/contact/forms/\(id)/emails/\(mail.id)/edit/", className: "edit", permission: "contact:forms:update"),
                                    .init(title: "Remove", href: "/admin/contact/forms/\(id)/emails/\(mail.id)/remove/", className: "delete", permission: "contact:forms:update")
                                ], permissions: ["contact:forms:update"]))
                            }
                        }
                    }
