...
struct ContactFormMailAdd: Component {
    ...
    func content() -> some BasicTag {
        Section { AdminContactFormTabs(formId: formId, active: .emails); AdminBreadcrumb(state: breadcrumb); H1("Add contact form email"); replacementVariables; ContactFormMailForm(mail: mail, action: "/admin/contact/forms/\(formId)/emails/add/", submitLabel: "Add email", error: error) }.class("cms-section")
    }
