...
struct ContactFormItemAddView: Component {
    struct State { let formId: String; let key: String; let type: String; let label: String; let allowedValues: String; let isRequired: Bool; let position: String; let error: String?; let breadcrumb: AdminBreadcrumb.State }
    let state: State
        ...
        Section {
            ...
            Form {
                Label {
                    ...
                    Select {
                        if state.type == "text" { Option("Text").value("text").selected() } else { Option("Text").value("text") }
                        if state.type == "textarea" { Option("Textarea").value("textarea").selected() } else { Option("Textarea").value("textarea") }
                        if state.type == "select" { Option("Select").value("select").selected() } else { Option("Select").value("select") }
                        if state.type == "radio" { Option("Radio").value("radio").selected() } else { Option("Radio").value("radio") }
