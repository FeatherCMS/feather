...
struct AdminAddContactFormItemOpenAPIRepository {
    ...
    func createItem(formId: String, form: ContactFormItemAddForm) async throws {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            let response = try await client.contactFormItemCreate(path: .init(contactFormId: formId), body: .json(.init(key: form.key, _type: form.type, label: form.label, allowedValues: form.normalizedAllowedValues, isRequired: form.isRequiredValue)))
            switch response {
            case .created: return
