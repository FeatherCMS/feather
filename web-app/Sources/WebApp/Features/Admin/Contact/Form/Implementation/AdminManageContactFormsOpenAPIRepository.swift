...
struct AdminManageContactFormsOpenAPIRepository {
    ...
    func get(id: String) async throws -> AdminManageContactFormItem {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            ...
            case .ok(let value):
                ...
                return .init(id: item.id, name: item.name, successMessage: item.successMessage, failureMessage: item.failureMessage, redirectUrl: item.redirectUrl, selectedFieldIDs: Set((item.items ?? []).map(\.id)), availableFields: fields, mails: (item.mails ?? []).map { .init(id: $0.id, mailFrom: $0.mailFrom, mailTo: $0.mailTo, subject: $0.subject, additionalHeaders: $0.additionalHeaders, messageBody: $0.messageBody) })
    ...
    func update(id: String, name: String, successMessage: String, failureMessage: String, redirectUrl: String?, fieldIDs: [String], mails: [AdminManageContactFormMail]) async throws -> AdminManageContactFormItem {
        try await api.withOpenAPIRepositoryErrorMapping { client in
            ...
            switch response {
            case .ok(let value): let item = try value.body.json; return .init(id: item.id, name: item.name, successMessage: item.successMessage, failureMessage: item.failureMessage, redirectUrl: item.redirectUrl, selectedFieldIDs: Set((item.items ?? []).map(\.id)), availableFields: [], mails: (item.mails ?? []).map { .init(id: $0.id, mailFrom: $0.mailFrom, mailTo: $0.mailTo, subject: $0.subject, additionalHeaders: $0.additionalHeaders, messageBody: $0.messageBody) })
