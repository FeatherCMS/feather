...
private typealias AnalyticsComponents = AnalyticsAdminAPI.Components
    ...
    @unchecked Sendable
{
    ...
    func webOverview(
        from: Double,
        to: Double
    ) async throws -> AnalyticsComponents.Schemas.AnalyticsLogOverviewSchema {
...
private func sampleOverview(
    paths: [AnalyticsComponents.Schemas
        .AnalyticsLogOverviewBreakdownItemSchema] = [
            .init(label: "/home", count: 10, share: 0.5)
        ]
) -> AnalyticsComponents.Schemas.AnalyticsLogOverviewSchema {
    .init(
        query: .init(
            source: "web_app",
            from: 1_718_150_400,
