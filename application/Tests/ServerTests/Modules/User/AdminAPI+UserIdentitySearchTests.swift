import FeatherHTTP
import HTTPTypes
import OpenAPIRuntime
import Testing

import UserAdminAPI

#if canImport(FoundationEssentials)
import FoundationEssentials
#else
import Foundation
#endif

@testable import Server

@Suite
struct AdminAPIUserIdentitySearchTests {

    @Test
    func searchUserIdentitiesIsOkForAuthorizedUser() async throws {
        let runner = try await TestRunner()
        try await runner.setupMigratedDatabase()
        try await runner.grantRootPermissions([
            "user:accounts:create",
            "user:accounts:list",
        ])
        let token = try await runner.authenticateTestAccount()

        let _ = try await runner.run(
            request: JSONRequest(
                method: .post,
                path: "/api/v1/admin/user/identities",
                headerFields: [
                    .authorization: runner.bearerAuthorizationHeader(
                        token: token
                    )
                ],
                body: Components.Schemas.UserIdentityCreateSchema(
                    status: .invited
                )
            )
        ) { response in
            try await response.json(
                status: .created,
                Components.Schemas.UserIdentityDetailSchema.self
            )
        }

        try await runner.run(
            request: JSONRequest(
                method: .post,
                path: "/api/v1/admin/user/identities/search",
                headerFields: [
                    .authorization: runner.bearerAuthorizationHeader(
                        token: token
                    )
                ],
                body: Components.Schemas.UserIdentityListItemSearchQuerySchema(
                    page: .init(size: 10, number: 1),
                    sort: [],
                    filters: .init(search: nil)
                )
            )
        ) { response in
            let object = try await response.json(
                status: .ok,
                Components.Schemas.UserIdentityListItemSearchSchema.self
            )
            #expect(object.data.total >= 1)
        }
    }

    @Test
    func searchUserIdentitiesFiltersByRoleAndReturnsRoleNames() async throws {
        let runner = try await TestRunner()
        try await runner.setupMigratedDatabase()
        try await runner.grantRootPermissions([
            "user:accounts:create",
            "user:accounts:update",
            "user:accounts:list",
            "user:roles:create",
        ])
        let token = try await runner.authenticateTestAccount()
        let roleID = "role-\(UUID().uuidString.lowercased())"
        let roleName = "Filter \(UUID().uuidString.prefix(8))"

        let role = try await runner.run(
            request: JSONRequest(
                method: .post,
                path: "/api/v1/admin/user/roles",
                headerFields: [
                    .authorization: runner.bearerAuthorizationHeader(
                        token: token
                    )
                ],
                body: Components.Schemas.UserRoleCreateSchema(
                    id: roleID,
                    name: roleName,
                    notes: nil
                )
            )
        ) { response in
            try await response.json(
                status: .created,
                Components.Schemas.UserRoleDetailSchema.self
            )
        }

        let identity = try await runner.run(
            request: JSONRequest(
                method: .post,
                path: "/api/v1/admin/user/identities",
                headerFields: [
                    .authorization: runner.bearerAuthorizationHeader(
                        token: token
                    )
                ],
                body: Components.Schemas.UserIdentityCreateSchema(
                    status: .invited
                )
            )
        ) { response in
            try await response.json(
                status: .created,
                Components.Schemas.UserIdentityDetailSchema.self
            )
        }

        _ = try await runner.run(
            request: JSONRequest(
                method: .patch,
                path: "/api/v1/admin/user/identities/\(identity.id)",
                headerFields: [
                    .authorization: runner.bearerAuthorizationHeader(
                        token: token
                    )
                ],
                body: Components.Schemas.UserIdentityPatchSchema(
                    roleIds: [role.id]
                )
            )
        ) { response in
            try await response.json(
                status: .ok,
                Components.Schemas.UserIdentityDetailSchema.self
            )
        }

        let result = try await runner.run(
            request: JSONRequest(
                method: .post,
                path: "/api/v1/admin/user/identities/search",
                headerFields: [
                    .authorization: runner.bearerAuthorizationHeader(
                        token: token
                    )
                ],
                body: Components.Schemas.UserIdentityListItemSearchQuerySchema(
                    page: .init(size: 10, number: 1),
                    sort: [],
                    filters: .init(role: roleName)
                )
            )
        ) { response in
            try await response.json(
                status: .ok,
                Components.Schemas.UserIdentityListItemSearchSchema.self
            )
        }

        #expect(result.data.total == 1)
        #expect(result.data.items.first?.id == identity.id)
        #expect(result.data.items.first?.roles == [roleName])
    }
}
